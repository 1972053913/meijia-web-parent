<template>
    <section>
        ...
        <el-dialog title="新增" :visible.sync="addFormVisible" :close-on-click-modal="false">
            <el-form :model="addForm" label-width="80px" :rules="addFormRules" ref="addForm">
                ...
                <el-form-item label="商品详情">
                    ...
                    <SquillEditorFastdfs host="http://192.168.43.155" v-model="addForm.content" uploadUrl='http://localhost:9527/services/common/file/upload'></SquillEditorFastdfs>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
        ...
        <el-dialog title="SKU属性管理" :visible.sync="skuDialogVisible" width="60%">
            ...
            <el-table :data="skus" border style="width: 100%" v-show="skus.length">
    ...
    export default {
        ...
        methods: {
            ...
            handleSkuProperties(){
                ...
                //价格库存的回填
                this.$http.get("/product/sku/skuByProductId?productId="+productId).then(res=>{
                    ...
                    for(let i=0;i<sku.length;i++){
                        for(let j=0;j<data.length;j++){
                            if(sku[i].sku_index==data[j].skuIndex){
                                sku[i].price=(data[j].price).toString();
                                sku[i].availableStock=(data[j].availableStock).toString();
                            }
                        }
                    }
                })
            ...
            handleDel: function (index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    //NProgress.start();
                    let para = row.id;
                    this.$http.delete("/product/product/"+para,{}).then((res) => {
                        this.listLoading = false;
                        this.$message({
                            message: '删除成功',
                            type: 'success'
            ...
            handleEdit: function (index, row) {
                ...
                var id=this.addForm.productTypeId
                ...
                if(row.medias){
                    // let arr=new Array();
                    var data1=row.medias.replace('[','').replace(']','').replace('"','').replace('"','').replace('"','').replace('"','').split(",");
                    for(let i=0;i<data1.length;i++){
                        //将数据添加到数组中
                        // arr.push(data1[i])
                        let temp = {};
                        temp.name=data1[i];
                        temp.url="http://192.168.43.155"+data1[i];
                        this.mediaList.push(temp)
                    }
                    // console.debug(this.mediaList)
                }
            },
            //回填时tree的处理
            changeDetSelect(key,treeData){
                ...
                this.addForm.productTypeId = childrenEach(treeData, depth);
            },
            //显示新增界面
            handleAdd: function () {
                this.addFormVisible = true;
